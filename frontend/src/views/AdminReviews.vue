    ...
    <div class="admin-card">
      ...
      <div v-if="candidates.length" class="candidate-list">
        <article v-for="candidate in candidates" :key="candidate.userId" class="candidate-card">
          ...
          <button class="template-button small save-memo-button" @click="saveMemo(candidate.userId)">메모 저장</button>
  ...
  try {
    ...
    const [summaryRes, candidatesRes] = await Promise.all([
      ...
      axios.get(`${baseURL}/admin/reviews`, {
        headers: getHeaders(),
        params: { status: statusFilter.value, dueSoonOnly: dueSoonOnly.value },
      }),
    ]);
